...
const READER_HTML_JAVASCRIPT: &str = include_str!("html/reader_javascript.html"); // Default HTML/JavaScript template
...
pub fn generate_reader_javascript(path: &str, template_path: &Option<String>) -> std::io::Result<()> {
        let mut entries: Vec<_> = fs::read_dir(path)?
        .filter_map(Result::ok)
        .filter(|entry| {
                let path = entry.path();
        ...
        if let Some(path) = template_path {
                ...
                template = READER_HTML_JAVASCRIPT.to_string();
                println!("Default reader template loaded.");
        }
        ...
        let mut javascript_array = String::new();
        javascript_array += "[";
        for entry in entries {
                let entry = format!("'{}',", entry.file_name().to_string_lossy().to_string());
                javascript_array += &entry
        }
        javascript_array += "]";
        template = template.replace("$ARRAY", &javascript_array);
	template = template.replace("$END", END);

	let html_path = Path::new(path).join(format!("reader.html"));
	let mut file = File::create(html_path)?;
